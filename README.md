...
## Rencana nilai (kalkulator nilai + prioritas belajar)

- **Halaman `/belajar/nilai`** (link dari hub Belajar): per mata kuliah, masukkan komponen nilai (nama, bobot %, nilai) sesuai rubrik dosen. Dihitung otomatis: nilai berjalan, proyeksi nilai akhir, dan kalkulator "butuh berapa di sisa komponen supaya nilai akhir sekian".
- **Ranking prioritas belajar**: mata kuliah diurutkan dari komponen belum dinilai dengan bobot terbesar (logika murni di `src/lib/grade-calc.ts`, teruji) — itu yang paling menentukan nilai akhir, jadi diprioritaskan duluan.
- Tabel baru: `grade_components` (migrasi `20260922010000_grade_components.sql`), RLS per pengguna, ikut terhapus kalau mata kuliahnya dihapus.
- Halaman ruang belajar per materi (`/belajar/$id`) sekarang lepas dari batas lebar 1280px standar (prop `wide` di `StudyShell`), supaya PDF dan poin materi tidak kekecilan di layar besar.
- **Prioritas rencana harian ikut bobot nilai**, tapi hanya komponen yang relevan sekarang — misalnya di musim UTS, dorongan prioritas ikut bobot UTS, bukan bobot UAS yang mungkin lebih besar tapi belum waktunya. Dicocokkan lewat nama komponen (mengandung "uts"/"uas") terhadap `exam_scope` materi atau `exam_kind` ujian; komponen "Tugas/Kuis/Aktivitas" mendorong item tugas biasa.
- **Impor foto rubrik nilai**: tombol "Pilih atau foto rubrik" di dialog Impor cepat, baca satu atau beberapa mata kuliah sekaligus dari satu foto lewat Gemini, hasilnya teks yang sama seperti impor cepat manual (bisa diperiksa dulu sebelum diterapkan). Pemakaian AI: kind `nilai`.
