...
function TimerPage() {
  ...
  const reset = (m = minutes, nextMode = mode) => {
    setRunning(false);
    setMinutes(m);
    setSeconds(m * 60);
    setMode(nextMode);
    ...
    <StudyShell title="Timer belajar" kicker="Saatnya fokus">
      <div className="grid gap-5 lg:grid-cols-[1.5fr_.8fr]">
        <PaperCard className="relative overflow-hidden text-center">
          ...
          <div className="mx-auto flex w-fit rounded-md bg-muted p-1">
            <Button
              variant={mode === "focus" ? "default" : "ghost"}
              size="sm"
              onClick={() => reset(25, "focus")}
            >
              Fokus
            </Button>
            <Button
          ...
          >
            <div>
              ...
              <small>{mode === "focus" ? "tetap fokus, ya" : "tarik napas dulu"}</small>
            </div>
          </div>
        ...
        <div className="space-y-5">
          <PaperCard>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-display text-3xl font-bold">7 hari</p>
              </div>
            </div>
            <div className="mt-5 flex justify-between">
              {[1, 1, 1, 1, 1, 1, 1].map((x, i) => (
                <div key={i} className="text-center">
                  <span className="block h-7 w-7 rounded-full bg-primary text-sm leading-7 text-primary-foreground">
                    ✓
                  </span>
          ...
          <PaperCard>
            ...
            <p className="mt-4 text-4xl font-bold">
              50 <span className="text-base font-medium text-muted-foreground">menit</span>
            </p>
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-muted">
              <div className="h-full w-2/3 bg-accent" />
            </div>
            <p className="mt-2 text-xs text-muted-foreground">2 dari 3 sesi selesai</p>
          </PaperCard>
        </div>
      </div>
    </StudyShell>
